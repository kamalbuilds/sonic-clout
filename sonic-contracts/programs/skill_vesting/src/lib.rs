...
declare_id!("285YigWq8Sy2nqFwVfVFJyoMFjhCvSXLwfNwyEpFufBR");
...
pub mod skill_vesting {
    ...
    pub fn check_milestones(ctx: Context<CheckMilestones>) -> Result<()> {
        ...
        let mut total_unlock_amount = 0;
        let total_amount = vesting_schedule.total_amount;
        let vesting_id = vesting_schedule.vesting_id;
        ...
        for (i, milestone) in vesting_schedule.milestones.iter_mut().enumerate() {
            if !milestone.reached && current_value >= milestone.threshold {
                ...
                let unlock_amount = (total_amount * milestone.unlock_percentage) / 10000;
        ...
        // Update unlocked amount if any milestones were reached
        if new_milestone_reached {
            vesting_schedule.unlocked_amount += total_unlock_amount;
            Ok(())
        } else {
            err!(VestingError::NoNewMilestonesReached)
        }
    }
    ...
    pub fn withdraw_unlocked(ctx: Context<WithdrawUnlocked>) -> Result<()> {
        ...
        // Instead of trying to call check_milestones, we'll implement the logic directly
        let oracle_data = ctx.accounts.oracle.try_borrow_data()?;
        let current_value = u64::from_le_bytes(oracle_data[0..8].try_into().unwrap());
        drop(oracle_data);
        ...
        let seed_prefix = b"token_vault";
        let vesting_id_bytes = vesting_schedule.vesting_id.to_le_bytes();
        let seeds = &[
            seed_prefix,
...
#[derive(Accounts)]
#[instruction(amount: u64, metric_type: String, thresholds: Vec<u64>, unlock_percentages: Vec<u64>)]
